...
class LandlineSucessPage extends StatelessWidget {
  const LandlineSucessPage({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(
        child: BlocBuilder<LandlineCubit, LandlineRechargeState>(
          builder: (context, state) {
            return Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                state is LandlineRechargeSelected
                    ? const Icon(Icons.check_circle,
                        color: Colors.green, size: 100)
                    : const Icon(Icons.warning, color: Colors.red, size: 100),
                const SizedBox(height: 8),
                state is LandlineError
                    ? const Text(
                        "Action Halted!",
                        style: TextStyle(
                          fontSize: 16,
